...
for i in range(n_traj):
    ...
    phase.set_state_options('p%dy' % i,
                            scaler=0.01, defect_scaler=0.1, fix_initial=True, 
                            fix_final=True)
    phase.set_state_options('p%dvx' % i,
                            scaler=0.01, defect_scaler=0.1, fix_initial=True, 
                            fix_final=True)
    phase.set_state_options('p%dvy' % i,
                            scaler=0.01, defect_scaler=0.1, fix_initial=True, 
                            fix_final=True)
    ...
    phase.add_control('p%dthrust' % i, rate_continuity=False, units='N', 
                      opt=True, upper=30, lower=-30, scaler=1.0)
...
for i in range(n_traj):
    ...
    p['phase0.states:p%dvx' % i] = phase.interpolate(ys=[0.0, 0.0], nodes='state_input')
    p['phase0.states:p%dvy' % i] = phase.interpolate(ys=[0.0, 0.0], nodes='state_input')

    p['phase0.states:p%dmass' % i] = phase.interpolate(ys=[50.0, 50.0], nodes='state_input')
...
for i in range(n_traj):
    ...
    thrust = exp_out.get_val('phase0.timeseries.controls:p%dthrust' % i)
    ...
    print()
    data[i] = {'x' : x, 'y' : y, 'thrust' : thrust, 'mass' : mass, 
